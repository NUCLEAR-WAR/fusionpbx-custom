...
if [ $switch_branch = "stable" ]; then
	...
	if [ $(echo "$switch_version" | tr -d '.') -gt 1100 ]; then
		...
		patch -u /usr/src/freeswitch-$switch_version/src/switch_rtp.c -i /usr/src/fusionpbx-custom/debian/resources/switch/source/switch_rtp.diff
		...
		patch -d /usr/src/freeswitch-$switch_version/src -i /usr/src/fusionpbx-custom/debian/resources/switch/source/pull_2300.diff

		#apply mod_pgsql patch
		#patch -u /usr/src/freeswitch-$switch_version/src/mod/databases/mod_pgsql/mod_pgsql.c -i /usr/src/fusionpbx-custom/debian/resources/switch/source/mod_pgsql.patch
	fi
fi
